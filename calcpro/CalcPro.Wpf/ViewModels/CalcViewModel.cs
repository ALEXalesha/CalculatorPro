...
public sealed partial class CalcViewModel : ObservableObject
{
    ...
    [RelayCommand] private void PressMemory(string op)
    {
        var mop = op switch
        {
            "M+" => MemoryOp.Add,
    ...
    private void UseHistoryEntry(HistoryEntry? entry)
    {
        ...
        if (!decimal.TryParse(entry.Result, NumberStyles.Float,
                CultureInfo.InvariantCulture, out var value)) return;
        _state.Display = entry.Result;
        _state.Phase = CalcPhase.EnteringDigit;
        SyncFromState();
    }
    ...
    private void PasteResult()
    {
        try
        {
            ...
            {
                _state.Display = v.ToString(CultureInfo.InvariantCulture);
                _state.Phase = CalcPhase.EnteringDigit;
                SyncFromState();
            }
        }
