...
Окно сжимается до 280×500 - меньше, чем у встроенного калькулятора Windows (320×500). В низком окне табло и отступы уменьшаются, и двенадцать рядов научного режима остаются по 23 px, а не 18; длинный пример в истории переносится после знаков, а не кончается «…». e2e сжимает окно до 280×500 и проверяет каждую клавишу каждого режима.

## Окно запоминает себя

Окно открывается там и такого размера, где его закрыли (`window-state.json` в папке данных приложения). Правило лежит в `window-state.js`, тот же файл у Calculator iOS 26: окно на отключённом мониторе встаёт по центру, больше экрана - ужимается, заголовок всегда остаётся на экране.
